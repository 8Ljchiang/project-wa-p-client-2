...
import { ISession, DataTypes, IProject } from '../../core/types/data-type-definitions';
...
export default class SessionDetailPage extends Component<ISessionDetailPage, {}> {
	render() {
		...
		const allSessions = autoGenerateMockData<ISession>(DataTypes.Session, 10);
		console.log(allSessions);
		const session = allSessions.filter((s: ISession) => {
			return sessionId === s.id;
		})[0];

		const allProjects = autoGenerateMockData<IProject>(DataTypes.Project, 10);
